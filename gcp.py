...
VM_PER_REGION = 4
MAX_PROJECT = 3
TARGET_TOTAL = MAX_PROJECT * VM_PER_REGION * 2
...
"torvello","zantrick","vorlento","trenvico"
]
...
def draw_ui(done,total,tokyo,osaka,status):
    ...
    print("ScriptV14: Tiến trình đang thực hiện ...\n")
    ...
    with open(OUTPUT_FILE,"w") as f:
        ...
        for p in projects[:MAX_PROJECT]:
            ...
            for i in inst:
                ...
                "--project",p,
                "--zone",zone,
                "--format=value(metadata.items.proxy_user,metadata.items.proxy_pass)"
                ])
...
def main():
    ...
    try:
        ...
        for p in projects:
            ...
            draw_ui(created,TARGET_TOTAL,tokyo,osaka,"Đang kiểm tra project...")

            if tokyo>=4 and osaka>=4:

                draw_ui(created,TARGET_TOTAL,tokyo,osaka,"Project đã đủ VM")
            ...
            for z in TOKYO_ZONES:
                ...
                while tokyo<4:
                    ...
                    draw_ui(created,TARGET_TOTAL,tokyo,osaka,"Đang tạo VM Tokyo")
            ...
            for z in OSAKA_ZONES:
                ...
                while osaka<4:
                    ...
                    draw_ui(created,TARGET_TOTAL,tokyo,osaka,"Đang tạo VM Osaka")
